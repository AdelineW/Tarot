              ...
              if (count == numero_spread) {
                ...
                setTimeout(() => {
                 ...
                 /* jQuery(document).ready(function ($) {
                  ...
                  jQuery(document).ready(function ($) {
  // Vérifier si on doit utiliser Dust ou Tarokina
  if (typeof window.userQuestion !== 'undefined' && window.userQuestion !== '') {
    // Utiliser Dust
    console.log('Envoi vers Dust avec question:', window.userQuestion);
    
    // Récupérer les noms des cartes depuis IdsPost
    var cardNames = [];
    if (typeof IdsPost !== 'undefined') {
      var cardIds = IdsPost.split(',');
      cardIds.forEach(function(cardId) {
        // Trouver l'élément de carte correspondant à cet ID
        var cardElement = document.querySelector('[data-id="' + cardId + '"], #carta' + cardId + ', .carta-' + cardId);
        if (cardElement) {
          var cardName = cardElement.querySelector('img') ? 
                        (cardElement.querySelector('img').alt || cardElement.querySelector('img').title) :
    ...
    console.log('Cartes récupérées:', cardNames);
    
    // Appel AJAX vers Dust
    jQuery.ajax({
        type: "post",
        url: dust_ajax.ajax_url,
        data: {
          action: 'send_to_dust',
          question: window.userQuestion,
          cards: cardNames,
          nonce: dust_ajax.nonce
        },
        success: function(response){
          console.log('Réponse Dust:', response);
          if (response.success) {
            var html = '<div class="dust-tarot-response">';
            html += '<h3>Votre guidance spirituelle</h3>';
            ...
            $('#results').html('<div class="error">Erreur Dust : ' + response.data + '</div>');
          }
        },
        error: function() {
          $('#results').html('<div class="error">Erreur de connexion avec Dust</div>');
        }
    });
    
  } else {
    // Utiliser la réponse Tarokina originale
    console.log('Pas de question Dust, utilisation de Tarokina');
    jQuery.ajax({
        type: "post",
        url: ajax_tarot.url,
        data: "action=" + ajax_tarot.action + "&nonce=" + ajax_tarot.nonce + "&cards="+ IdsPost + "&nametarot="+ nametarot + "&flip="+ flip_arr,
