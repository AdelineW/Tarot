...
self.removeDragScrollData();$usb.postMessage('doAction','_scrollDragging')}});$.extend(prototype,{updateElementsList:function(){const self=this;const content=$ush.toString(self.pageData.content);const allShortcodesRegexp=self.getShortcodeRegex('[\dA-z_]+');const parseShortcode=(content,parentId,depth)=>{depth=$ush.parseInt(depth);if(!content||depth>1000){return}
Array.from(content.matchAll(allShortcodesRegexp)).forEach((matches)=>{const shortcodeId=(matches[3].match(/(\s?usbid="([^\"]+)?")/)||[])[2];const shortcodeContent=$ush.toString(matches[5]);if(!shortcodeId){return}
