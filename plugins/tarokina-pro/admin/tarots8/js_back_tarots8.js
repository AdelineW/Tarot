window.addEventListener("load", function () {
  ...
  let wp_content = back_tarots8.wp_content;

  let addons_inactive = back_tarots8.addons_inactive;
  ...
  for (var i = 0; i < tabs.length; i++) {
    let Tarot_key = document.getElementsByName(
      "carbon_fields_compact_input[_tarokki_tarot_complex8][" + i + "][value]"
    );
    let Tarot_ID = document.getElementsByName(
      "carbon_fields_compact_input[_tarokki_tarot_complex8][" +
        i +
        "][_tkta_id]"
    );
  ...
  btn_guardar.value = back_tarots8.save_text;
  ...
  trashArr.forEach(function (trashX) {
    trashX.addEventListener("click", function (e) {
      ...
      btn_guardar.value = back_tarots8.textDelete;
