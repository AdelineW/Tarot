<?php
			...
			if ($pageId == 'tarokkina_pro_page_crb_carbon_fields_container_tarots') {
				$tarots = carbon_get_theme_option('tarokki_tarot_complex') ?? [];
			}else{
				$tarots = get_option('tarokki_tarotsAll') ?? [];
			}
	
			foreach ( (array) $tarots as $tarot) {
				...
				update_option( $nombreSC, array_fill_keys($tarotsN,$tarots) );
					...
					foreach ($Posts_ID as $Post_ID) {
						...
						if ($cardImg == false ) {
							...
							$cards_ssl[$Post_ID]= str_replace('http://','https://',plugin_dir_url(dirname( __FILE__,2 ) ).'img/no-card.png');
							$cardsFull_ssl[$Post_ID]= str_replace('http://','https://', plugin_dir_url(dirname( __FILE__,2 ) ).'img/no-card.png');
			...
			$guadandoNames = (get_option( 'tarokki_tarot_names')!== false) ? get_option( 'tarokki_tarot_names') : array() ;
			...
			update_option( 'tarokki_tarot_names',$tarotsN2);
			update_option( 'tarokki_tarotsAll',$tarotsAll);
			// array con key=producto_id, value= Time tarot restriction
			update_option( 'tarokki_EDD_products',$restrict_post_id);
