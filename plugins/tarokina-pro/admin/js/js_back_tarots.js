window.addEventListener("load", function () {
  ...
  // Verificar que ambos elementos existen antes de realizar la sustitución
  if (text_holder && complex_placeholder.length > 0) {
    // Obtener el texto del elemento text_holder
    const textContent = text_holder.textContent || text_holder.innerText;
    
    // Aplicar el texto a todos los elementos con la clase complex_placeholder
    for (let i = 0; i < complex_placeholder.length; i++) {
      complex_placeholder[i].textContent = textContent;
    }
  }


  let wp_content = back_tarots.wp_content;

  let addons_inactive = back_tarots.addons_inactive;
  ...
  for (var i = 0; i < tabs.length; i++) {
    let Tarot_key = document.getElementsByName(
      "carbon_fields_compact_input[_tarokki_tarot_complex][" + i + "][value]"
    );
    let Tarot_ID = document.getElementsByName(
      "carbon_fields_compact_input[_tarokki_tarot_complex][" + i + "][_tkta_id]"
    );
  ...
  btn_guardar.value = back_tarots.save_text;
    ...
    const inputFields = document.querySelectorAll('input[name$="[_tkta_name]"]');
  ...
  trashArr.forEach(function (trashX) {
    trashX.addEventListener("click", function (e) {
      ...
      btn_guardar.style.background = 'red';
      btn_guardar.value = back_tarots.textDelete;
  ...
  Array_short.forEach(function (nameValue) {
    const position =
      "carbon_fields_compact_input[_tarokki_tarot_complex][" +
      num +
      "][_tkta_name]";
