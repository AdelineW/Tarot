<?php
 ...
 : '' ;
        $Table_Row_basic [] = '<div class="table-row">
        <div class="table-data">
          <span class="numTitle">'.esc_html($i).'</span>&nbsp;'.esc_html($the_card).'
        </div>
        </div>';
    };
    $Table_Row_basic = implode($Table_Row_basic );
    ...
    Container::make( 'post_meta', '<span $style class=\"t_mode\"><span class=\"spre\">".esc_html__('Tarot','tarokina-pro')."</span></span>".esc_html($tirada)."' )
                ...
                <div class=\"cab_cartaA\">  
                 ...
                 <div class=\"cab_cartaB\">
                        ...
                        <table>
                            ...
                            <tr>
                            <td class=\"td1Box\">'.esc_html__( 'Card', 'tarokina-pro' ).':</td> <td class=\"tdCard\">'.esc_html(\$title).'</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                            <td class=\"svgTex td1Box\">'.esc_html__( 'Texts', 'tarokina-pro' ).':</td> <td class=\"td2Box\">1</td>
                            </tr>
                        </table>
                        </div>
                    </div>
        ...
        Field::make( 'radio', 'tkta_yesno_".$id_unique."expert0', '' )->add_options( array(
            ...
            Field::make( 'rich_text', 'tkta_text_".$id_unique."expert0',  $tabTitle )->set_classes( 'text_F0' ),
        ) );";

        $secciones_carta_basic = [$ALL_CONTAINER_BASIC];
        $vista_carta_basic= implode($secciones_carta_basic);
        $escribir_basic = fopen( TAROKINA_ADMIN_PATH. 'cartas/carta_vista.php', "a+" );
        fwrite($escribir_basic, $vista_carta_basic . PHP_EOL);
        fclose($escribir_basic);
