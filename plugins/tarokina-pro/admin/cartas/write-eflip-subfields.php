<?php
    // restar los 3 que estan el la variable Top
    $numLoop = $spread_N - 2;
 ...
 : '' ;
    ...
    Container::make( 'post_meta', '<span $style class=\"t_mode\"><span class=\"spre\">".esc_html__('Tarot','tarokina-pro')."</span></span>".esc_html($tirada)."' )
        ...
        ->add_fields(array(
            ...
            <div class=\"cab_carta\">
                    ...
                    <div class=\"cab_cartaB\">
                        ...
                        <table>
                            ...
                            <tr>
                            <td class=\"svgTex td1Box\">'.esc_html__( 'Texts', 'tarokina-pro' ).':</td> <td class=\"td2Box\">".esc_html($spread_N * 2)."</td>
                            </tr>
                        </table>
                        </div>
                    </div>
            ...
            <li class=\"text000\">'.esc_html__('Reversed expert Mode: Multiple texts are used per card (2 texts per reference on each card). If the card is reversed, the alternative text B will be used on each reference. There are twice as many texts as in Expert mode.','tarokina-pro').'</li>
            </ul>
            </div>
        </div>
        ' )->set_classes( 'text_F0' ),
        Field::make( 'rich_text', 'tkta_text_".$id_unique.'expert'."0', $tabTitle )
        ->set_classes( 'text_F0' ),
        Field::make( 'rich_text', 'tkta_text_".$id_unique.'inve_expert'."0', '1 - ". str_replace("'", "’", $tarot['tkta_'.esc_html(1).'st'])." - '.esc_html__( 'Reversed', 'tarokina-pro' ) )
        ->set_classes( 'text_F0 text_Finv' ),
        ...
        ' )->set_classes( 'text_F1' ),
        Field::make( 'rich_text', 'tkta_text_".$id_unique.'expert'."1', $tabTitle )
        ->set_classes( 'text_F1' ),
        Field::make( 'rich_text', 'tkta_text_".$id_unique.'inve_expert'."1', '2 - ". str_replace("'", "’", $tarot['tkta_'.esc_html(2).'st'])." - '.esc_html__( 'Reversed', 'tarokina-pro' ) )
        ->set_classes( 'text_F1 text_Finv' )";
    ...
    for ($i=0; $i < $numLoop ; $i++) { 
        ...
        Field::make( 'rich_text', 'tkta_text_".$id_unique.'expert'."$num_html', $tabTitle )
        ->set_classes( 'text_F".$class_F."' ),
        Field::make( 'rich_text', 'tkta_text_".$id_unique.'inve_expert'."$num_html', '$BigNum - ". str_replace("'", "’", $tarot['tkta_'.esc_html($BigNum).'st'])." - '.esc_html__( 'Reversed', 'tarokina-pro' ) )
        ->set_classes( 'text_Finv' )";
    }
