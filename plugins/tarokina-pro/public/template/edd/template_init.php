...
$fondoCard = ( $sp_title_color !== '') ? 'background-color: rgba('.$r.','.$g.','.$b.',0.05 )' : 'background-color: rgba(0, 0, 0, 0.03)';
...
<style>
    ...
    <?php
        if ( $sp_bg_color == '' && $img_fondo == array()) { 
...
<?php
if (in_array(get_the_id(), $ids_restriction) == false || $tkna_shop_id == "" || function_exists('run_tarokki_edd_restriction_tarokina') == false || function_exists('EDD')== false || $mycard_arr['edd_restriction_tarokina'] !== MYCARD || current_user_can( 'manage_options' )) { 
}else{
   ...
   if ( is_user_logged_in() && $logout == 0 ) {
       $history_page = edd_get_option('purchase_history_page', false );  
   ?>
           ...
           <div class="text-end">
               ...
               <div>
                   <h3 class="nameU"><?php echo $user->display_name?></h3>
                       <div class="taro_btns">
                           <a class="" href=" <?php echo get_permalink( $history_page) ;?>"> <?php echo esc_html__( 'Orders', 'tarokina-pro' ) ;?></a>&nbsp;|&nbsp;<a style="text-decoration:none!important" href="<?php echo wp_logout_url( get_permalink() )?>" tabindex="1" class=""><?php echo __( 'Logout', 'tarokina-pro' )?></a> 
                   </div>
               </div>   
           </div>
...
?>
<div id="tkna_preloader" data-version="<?php esc_attr_e(TAROKKINA_PRO_VERSION)?>" data-nametarot="<?php esc_attr_e($nombre)?>" data-n="<?php esc_attr_e($type_N)?>" class="_<?php esc_attr_e($cssYesNo)?>">
    <div style="<?php echo esc_attr($spreadImg.$ancho)?>" class="spread">
        <div style="background:<?php echo esc_attr($sp_bg_color) . esc_attr($opacity)?>" class="capaColor"></div>
         <?php  if ($title_spread) { ?>
            ...
            <?php }else{?>
            <h2 style="display:none" class="opT"></h2>
         <?php } ?>
        ...
        <div id="tablero" class="tablero" data-orden="<?php esc_attr_e(implode(",", $clickOrden))?>" <?php esc_attr_e($size_cards)?>>
        
            <?php for ($i=1; $i <= $type_N; $i++) { 
            echo '<div style="'.esc_attr( $fondoCard ).$back_imageSpread.'" id="molde'.$i.'" class="m'.$i.' molde">
    ...
    <?php
        ...
        if (in_array(get_the_id(), $ids_restriction) == false || $tkna_shop_id == "" || function_exists('EDD') == false || function_exists('run_tarokki_edd_restriction_tarokina') == false || $mycard_arr['edd_restriction_tarokina'] !== MYCARD || current_user_can( 'manage_options' )) { 

             ///////////////// Restriction  Desactivado ///////////////////////////
            require_once TAROKINA_PUBLIC_PATH . 'template/decks/selector.php';
        ...
        if ( $transient !== false && is_user_logged_in()) {  // Se ha comprado y existe Transient 
            ...
            $product = edd_get_download($tkna_shop_id);
            $productName = ($product !== false) ? $product->post_title : 'N/A';
            $productID = ($product !== false) ? $product->ID : '';
            ...
            if ($link_direct == 0) {
                $link_product = '<a href="' .get_permalink( $purchase_page) . '?edd_action=add_to_cart&download_id='.$productID.'">' . $productName . '</a>';
            }else{
            ...
            <p style="text-align:center">
              ...
              $modal = '
              <div class="woo_my_account loginRestri-container">
                ...
                <div class="loginRestri-content">
                  ...
                  '.do_shortcode( '[edd_login redirect="'.get_permalink( get_the_ID()).'"]' ).'
                </div>          
              </div>
              ';
          ...
          if ( ! is_user_logged_in() && $link_acount == 0 ) { 
            echo '<div class="woo_my_account">'.do_shortcode( '[edd_login]' ).'</div>';
          }  
       } 
    }
    ?>
