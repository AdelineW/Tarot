 ...
 * @version 1.9.4
 */
class TAROKKI_SL_Plugin_Updater {
	...
	public function __construct( $_api_url, $_plugin_file, $_api_data = null ) {
		...
		// Usar la URL real para el cache key (después de definir api_url)
		$request_url = $this->get_request_url();
		$this->failed_request_cache_key = 'edd_sl_failed_http_' . md5( $request_url );
	...
	/**
	 * Obtiene la URL correcta para hacer la petición, usando fallback si está configurado.
	 */
	private function get_request_url() {
		// Determinar si usar producción o desarrollo
		$use_production = (defined('TAROKINA_PRODUCTION_MODE') && TAROKINA_PRODUCTION_MODE);
		
		if ($use_production) {
			// En producción, usar la lógica de dominio dinámico
			$change_domain = get_option('_change_license_domain', '');
			if ($change_domain == 'yes') {
				return 'https://arnelio.com';
			} else {
				return 'https://edd-license-proxy.service-dd4.workers.dev';
			}
		} else {
			// En desarrollo, usar TAROKINA_URL_DEV si está definido, sino usar api_url original
			if (defined('TAROKINA_URL_DEV')) {
				return TAROKINA_URL_DEV;
			}
			return $this->api_url;
		}
	}
	...
	public function check_update( $_transient_data ) {
		...
		if ( false !== $current && is_object( $current ) && isset( $current->new_version ) ) {
			if ( version_compare( $this->version, $current->new_version, '<' ) ) {
				// FIX: No añadir al transient si no hay paquete disponible (licencia inválida)
				// Esto previene el mensaje "Automatic update is unavailable for this plugin"
				if ( empty( $current->package ) ) {
					// Si no hay paquete, no mostrar en transient para evitar mensaje confuso
					// WordPress no mostrará notificación de actualización sin paquete válido
					return $_transient_data;
				}
	...
	private function get_update_transient_data() {
		...
		$limited_data->icons        = isset( $version_info->icons ) ? $this->convert_object_to_array( $version_info->icons ) : array();
		$limited_data->banners      = isset( $version_info->banners ) ? $this->convert_object_to_array( $version_info->banners ) : array();
	...
	private function api_request( $_action, $_data ) {
		...
		// Don't allow a plugin to ping itself - check against the actual request URL
		$request_url = $this->get_request_url();
		if ( trailingslashit( home_url() ) === trailingslashit( $request_url ) ) {
			return false;
		}
	...
	private function get_version_from_remote() {
		...
		// Usar la URL correcta según el entorno (producción/desarrollo)
		$request_url = $this->get_request_url();
