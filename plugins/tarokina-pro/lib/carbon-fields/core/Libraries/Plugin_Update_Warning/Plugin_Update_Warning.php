...
namespace Carbon_Fields_Tarokina_Pro\Libraries\Plugin_Update_Warning;
...
class Plugin_Update_Warning {
	...
	protected function has_major_update() {
		...
		$carbon_fields_data = isset( $all_updates[ \Carbon_Fields_Tarokina_Pro\RELATIVE_PLUGIN_FILE ] ) ? $all_updates[ \Carbon_Fields_Tarokina_Pro\RELATIVE_PLUGIN_FILE ] : null;
		if ( ! $carbon_fields_data ) {
			return false;
		}

		$plugin_data = (object) _get_plugin_data_markup_translate( \Carbon_Fields_Tarokina_Pro\RELATIVE_PLUGIN_FILE, (array) $carbon_fields_data, false, true );
	...
	protected function get_json_data() {
		return array(
			'plugin_path' => \Carbon_Fields_Tarokina_Pro\RELATIVE_PLUGIN_FILE,
		);
	}
